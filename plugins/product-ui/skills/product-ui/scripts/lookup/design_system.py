#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Modified from ui-ux-pro-max-skill (MIT, Next Level Builder); licence: ../../data/LICENSE.ui-ux-pro-max.txt
"""
...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        return {
            ...
            "typography": {
                "heading": best_typography.get("Heading Font", ""),
                "body": best_typography.get("Body Font", ""),
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("")
    lines.append("> **STARTING POINT:** This file is lookup output for product-ui Step 1, which converts it into")
    lines.append("> `tokens.json` under `references/tokens-format.md` and `references/ban-list.md`. Colours become")
    lines.append("> `oklch()` values, fonts are checked against the ban list, and the components read `theme.css`.")
    lines.append("> A page file in `pages/` beside this one records where that page departs from this file.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("### Typography")
    lines.append("")
    lines.append(f"- **Heading Font:** {typography.get('heading') or 'no match; choose one in Step 1'}")
    lines.append(f"- **Body Font:** {typography.get('body') or 'no match; choose one in Step 1'}")
    if typography.get("mood"):
        lines.append(f"- **Mood:** {typography.get('mood', '')}")
    if typography.get("google_fonts_url"):
    ...
    lines.append("|-------|-------|-------|")
    lines.append("| `--shadow-sm` | `0 1px 2px oklch(0% 0 0 / 0.05)` | Subtle lift |")
    lines.append("| `--shadow-md` | `0 4px 6px oklch(0% 0 0 / 0.08)` | Cards, buttons |")
    lines.append("| `--shadow-lg` | `0 10px 15px oklch(0% 0 0 / 0.12)` | Modals, dropdowns |")
    lines.append("| `--shadow-xl` | `0 20px 25px oklch(0% 0 0 / 0.16)` | Hero images, featured cards |")
    lines.append("")
    ...
    lines.append("  box-shadow: 0 0 0 3px color-mix(in oklch, var(--color-primary) 20%, transparent);")
    lines.append("}")
    lines.append("```")
    lines.append("")
    ...
    lines.append("  background: oklch(0% 0 0 / 0.5);")
