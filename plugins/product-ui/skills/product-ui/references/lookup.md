...
Re-run with different keywords (`"playful neon"` → `"vibrant dark"` → `"content-first minimal"`); the data stays as it is. The web stack this skill builds with is settled in `references/stack-defaults.md`; a stack result from the lookup does not override it.
